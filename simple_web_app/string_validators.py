...
from .exceptions import (
    ...
    InvalidEspecialCharacterException,
)
...
class InvalidEspecialCharacterValidator(Validator):
    def __init__(self):
        self.especial_set = set("^~/")

    def validate(self, content):
        content_set = set(content)
        if self.especial_set.intersection(content_set):
            msg = "Passwords must not have ^, ~, /"
            raise InvalidEspecialCharacterException(detail=msg)
